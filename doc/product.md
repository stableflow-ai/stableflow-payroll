...
**Organization (admin).** `GET /v1/payroll/organizations/{id}` hydrates Organization Name and optional Logo URL (the item matching `id`, or the first row). Same validation as register organization. Add Member / Invite use the same buttons and dialogs as Team. Save (bottom right) posts `POST /v1/payroll/organizations/{id}` (`useUpdateOrganizationMutation`) and writes `organization.name` / `logo` onto the session.
