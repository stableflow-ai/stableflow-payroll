...
| Batch Payout | `/pay/batch` | CSV / Google Sheets / manual rows (max 50). Validate then preview. `POST /v1/pay/batch/quote\|swap\|submit`. Recipients are wallet addresses. Origin tokens are any `PAYOUT_SYMBOLS` token on `batchEnabled` chains (including native gas tokens). Near-chain NEAR is wrap.near like Single (UI/balance show native NEAR). |
