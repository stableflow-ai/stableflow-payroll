...
Props: `open`, `onClose`, `payable: PayableKey | null`, optional `initialNetPayById`.
...
Renders `PaymentByFormCard` with `payable` + `formLocked` (Form dropdown disabled; the matching row still comes from `GET /v1/payroll/payables`). Optional `initialNetPayById` seeds Details / quote `adjustments`. Closes on successful send (`onSettled`). `/pay/payroll` passes `{ type: "payroll", periodMonth }` from `GET /v1/payroll/salaries/next` `payment_date` (must match payables `period_month` exactly) and snapshots Next Payroll Net Pay into `initialNetPayById`. `/pay/expense` Open expense and Request Payments pass `{ type: "expense", batchId }` from the row's batch. `/pay/bonus` uses `{ type: "bonus", batchId }` from `GET /v1/payroll/bonuses/open`.
