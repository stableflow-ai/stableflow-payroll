...
Renders `PaymentByFormCard` with `payable` + `formLocked` (Form dropdown disabled; the matching row still comes from `GET /v1/payroll/payables`). Closes on successful send (`onSettled`). Current callers: `/pay/payroll` and `/pay/expense` (placeholder keys until those dashboards pass live ids); `/pay/bonus` uses `{ type: "bonus", batchId }` from `GET /v1/payroll/bonuses/open`.
