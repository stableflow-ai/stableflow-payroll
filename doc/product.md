...
| Batch Payout | `/pay/batch` | CSV / Google Sheets / manual rows (max 50). Validate then preview. `POST /v1/pay/batch/quote\|swap\|submit`. Recipients are wallet addresses. Origin tokens are EVM ERC-20 on `batchEnabled` chains (no native gas tokens). |
