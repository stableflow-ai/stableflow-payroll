...
- Every other store that must survive a reload uses Zustand `persist` (`batch-payout-commit-queue`, `intents-tokens`, `quick-pay-prefs`, `token-select-prefs`, `google-drive-session`, `google-auth-pending`).
