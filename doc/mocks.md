...
| `organization` | `src/mocks/organization.ts` | `useUpdateOrganizationMutation` | Settings → Organization save. Success writes `organization.id` / `name` / `logo` onto the session user. |
