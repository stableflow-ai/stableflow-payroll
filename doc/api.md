...
`GET /bonuses/open` returns `total_payout`, `total_count`, and `batches` (`title`, `volume`, `list` of operation items including `email`, `purpose`, and `description`). The Bonuses to be paid table keeps each batch as a grouped row: Bonus Title / Member / Purpose / Description / Bonus / Address / Email / Action. One member shows those fields on the row; more than one expands and shows `-` for Purpose / Description / Address / Email until expanded. Description uses the same Description cell as Expense (http(s) URLs are links with a receipt icon before each URL; empty cells are `-`). `volume` is the USD total; member `amount` / `symbol` are the payout. `list[].email` is mapped onto `BonusPendingMember` and carried into Pay Now. `status` `paying` / `processing` / `submitted` on any member shows Paying for that batch; anything else is Pay Now with that batch's `batch_id`. An empty batch list is the create-bonus empty state.
