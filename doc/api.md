...
| POST | `/v1/payroll/auth/google/login` | no | `GoogleLoginBody` (`id_token`) | `AuthSession` | `googleLogin` | `useGoogleLoginMutation` |
| POST | `/v1/payroll/auth/google/register` | no | `GoogleRegisterBody` (`id_token`, `inviteCode`, `name`, `organization`) | `AuthSession` | `googleRegister` | `useGoogleRegisterMutation` |
| POST | `/v1/payroll/auth/google/register/user` | no | `GoogleRegisterUserBody` | `AuthSession` | `googleRegisterUser` | `useGoogleInviteRegisterMutation` |
...
| POST | `/v1/payroll/organizations/high-priority/click` | yes | `ClickOrganizationHighPriorityBody` (`category`, `organization_id`) | — | `clickOrganizationHighPriority` | `useClickOrganizationHighPriorityMutation` |
...
| POST | `/v1/payroll/payouts/retry` | yes | `PayrollPayoutRetryParam` (`execution_item_id`, `organization_id`, `success_url`) | `PayrollPayment` | `retryPayrollPayout` | `useRetryPayrollPayoutMutation` / `useRetryPayoutItem` |
...
`POST /payouts/retry` is the History **Pay Again** path for a failed or expired execution item (Payroll detail drawer, Expense / Bonus / Operations history tables). Body is `execution_item_id` (history row `id` or `execution_item_id`, posted as a number; `execution_id` is not used), session `organization_id`, and `success_url`: `{origin}/pay/payroll/history/{executionId}` for Payroll, `{origin}/pay/expense/history` for Expense, `{origin}/pay/bonus/history` for Bonus, `{origin}/pay/{category}/history` for Operations. The mapper reuses `mapPayrollPayment` and throws `NO_PAY_URL` when the link is missing. On success the UI sends the browser to `pay_url` with `window.location.assign`. Checkout query params on return are ignored; Payroll reopens the drawer from the path, and the other surfaces land on that history list.

`memo` (≤ 200 characters) is accepted but is not in the Swagger contract. Optional `notification: { email?, slack? }` is sent from Single Payment when Notify Recipient is on; empty keys are omitted. The switch-off path omits `notification` entirely. Optional `team_member_id` is sent for **admin** Single Payment when the recipient matches a Team member (address book, pasted member wallet, or Team Pay Now). Employees omit it.
...
| POST | `/v1/payroll/salaries/pay/quote` | yes | `PayrollPayParam` | `PayablePayQuote` | `payPayrollSalaries` | `usePayablePayQuery` |
| POST | `/v1/payroll/expenses/pay/quote` | yes | `PayablePayBaseParam` + `batch_id` | `PayablePayQuote` | `payExpenseBatch` | `usePayablePayQuery` |
| POST | `/v1/payroll/bonuses/pay/quote` | yes | `PayablePayBaseParam` + `batch_id` | `PayablePayQuote` | `payBonusBatch` | `usePayablePayQuery` |
| POST | `/v1/payroll/operations/pay/quote` | yes | `PayablePayBaseParam` + `batch_id` + `category` | `PayablePayQuote` | `payOperationBatch` | `usePayablePayQuery` |
| POST | `/v1/payroll/pay/quote/notification` | yes | `PayableQuoteNotificationParam` (`quote_id`, `notification`) | — | `setPayableQuoteNotification` | `usePayableQuoteNotificationMutation` |
...
Every quote body sends `payer` and `refundTo`. For non-Zcash origins they are the same connected address. Native Zcash sends the shielded Unified address (`u1`) as `payer` and the transparent address (`t1` / `t3`) as `refundTo`. If either Zcash address is missing, the card does not POST quote and shows **Zcash refunds require a transparent address from Noir Wallet.** Expense and bonus quote `batch_id` in the body (not the path). The quote routes return `{ quote_id, batches: [{ quote_batch_id, batch }] }` (`quote_id` is a sibling of `batches`). A legacy `{ quote_id, batch }` payload is wrapped as a single-element `batches` array. The mapper copies `quote_id` onto each inner `PayrollBatch` and throws `ApiError(..., "NO_QUOTE_ID")` or `ApiError(..., "NO_BATCH_TX")` when the id is missing or any batch cannot be broadcast. `transaction.outputs` (`address`, `amount`, `amountRaw` / `amount_raw`) counts as broadcastable when `callData` is empty, so a native Zcash quote can be sent. `PaymentByFormCard` posts quote as `usePayablePayQuery` (`staleTime: 0`, `gcTime: 0`, no placeholder). You Pay / Est. Cost sum `total_source_amount` across `batches`; Total Valued / Details Total Value sum `payments[].destination_volume` across them. When `batches.length === 1`, Send uses `markBatchConsumed(quote_batch_id)` then `broadcastBatchPayout` (Zcash: Noir `sendTransaction` to `outputs[0]`), then `enqueueBatchPayoutCommit({ quoteId, quoteBatchId, txHash, formKey })`. When `batches.length > 1`, the card replaces Send Payment with one **Pay Now** row per batch and enables only the next unpaid batch in order (Batch Payment 1 first); each enabled row broadcasts that batch then enqueues submit and stays on the form until every batch is paid. Consumed tracking uses `quote_batch_id`. Expired or consumed single-batch quotes refetch quote. After a split batch has been paid, the card does not refetch the quote (remaining batches keep their calldata). Backend `transaction.approvals` are on the first split batch. After EVM approve, if on-chain allowance is still below the remaining unpaid `total_source_amount_raw` sum (`Insufficient approval amount`), Send toasts and POSTs quote again so the backend can rebuild `approvals` calldata unless a split batch was already paid (then it toasts **Insufficient approval.** and does not refetch). An insufficient-approval failure unmarks that `quote_batch_id` as consumed. Notify Recipient is not on the quote body. After `quote_id` is known, `PaymentByFormCard` POSTs `/v1/payroll/pay/quote/notification` with `{ quote_id, notification }`: `"all"` when every item is selected, otherwise a comma-separated list of `item_id`s (`"1,5"`). Turning Notify off after a successful POST for that `quote_id` sends `notification: ""`. Changing Notify does not re-quote. Send stays disabled while Notify is on and that POST is pending or has not succeeded. Payroll `adjustments` is `{ item_id, net_pay }[]` for **every** item (Details override or list `net_pay` / `amount`); expense / bonus / Operations quote bodies never include `adjustments`. `adjustments` is part of the quote query key. `payablePayBody` copies `refundTo` and payroll `adjustments` onto quote bodies. Expense and bonus bodies send `batch_id`. Operations bodies send `batch_id` and `category` (= payable `type`) and omit `period_month` / `timezone`. `/pay/payroll` Pay Now seeds `PaymentByFormCard` `initialNetPayById` from Next Payroll Net Pay so those values enter the quote as `adjustments`. `mapPayrollBatchPayment` reads `destination_volume`. Total Valued / Details Total Value stay `$0` until quote returns.
...
Recent payouts have no `page` in the contract, only `limit` (max 100). `usePayrollRecentPayoutsInfiniteQuery` requests `limit = page * 10` and keeps the new slice. `failedCount` is counted from loaded rows with status `failed`. `completed` maps to `paid`; `expired` stays `expired`. The list polls every 30s while any loaded row is `pending` (`refetchIntervalInBackground: false`).

`GET /salaries/next` returns `total_payout`, `recipients`, `payment_date`, `payable`, and `list` (`PayrollOperationItem`: name, address, email, symbol, network, amount, net_pay). An empty `list` maps to `null` so the page shows the create-payroll empty state. `payable` is a boolean; explicit `false` disables Pay Now (button copy **Not payday yet**, click is a no-op). Missing `payable` maps to `true`.
...
`GET /salaries/history/{execution_id}` returns the same summary plus `list` (`PayrollOperationExecutionItem`). Rows map `recipient` → address, `destination_symbol` / `destination_network` → payout preference, `amount` / `net_pay`, `status` (`completed` → `paid`; `expired` stays `expired`; `failed` stays `failed`), and `destination_tx_hash`. Failed count is counted from loaded `failed` rows. The item has no email field; that line is omitted when empty.
...
Recent payouts have no `page` in the contract, only `limit` (max 100). `useExpenseRecentPayoutsInfiniteQuery` requests `limit = page * 10` and keeps the new slice. `failedCount` is counted from loaded rows with status `failed`. `completed` maps to `paid`; `expired` stays `expired`. The list polls every 30s while any loaded row is `pending` (`refetchIntervalInBackground: false`).

`GET /expenses/open` returns `total_payout`, `total_count`, and `batches` (`title`, `volume`, `count`, `list` of operation items including `email`, `purpose`, and `description`). The Open expense table keeps each batch as a grouped row: Title / Name / Purpose / Description / Address / Email / Payout Preference / Amount. One member shows those fields on the row; more than one expands and shows `-` for Purpose / Description / Address / Email until expanded. Description uses the same Description cell as Request Payments (http(s) URLs are links with a receipt icon before each URL; empty cells are `-`). Batch `volume` is the header Amount; member `amount` / `symbol` / `network` are the payout. `list[].email` is mapped onto `ExpenseOpenRow` and carried into Pay Now. `status` `paying` / `processing` / `submitted` on any member shows Paying for that batch; anything else is Pay Now with that batch's `batch_id`.
...
`GET /expenses/history` is paginated (`page` / `pageSize`, max 100) and accepts `search` (name, address, or amount) plus `start_time` / `end_time` as Unix seconds. `useExpenseHistoryInfiniteQuery` loads the next page on scroll and sends the History tab search and last-30-days (or custom) range. Description / Receipt uses the same Description cell as Request Payments (http(s) URLs are links with a receipt icon before each URL; empty cells are `-`). History Status is Failed, Expired, Paid, or Pending (`processing` maps to pending). Failed and Expired rows with a numeric `execution_item_id` / `id` show **Pay Again** (`POST /payouts/retry`, `success_url` `{origin}/pay/expense/history`). The list polls every 5s while any row is `pending` (`refetchIntervalInBackground: false`).
...
Recent payouts have no `page` in the contract, only `limit` (max 100). `useBonusRecentPayoutsInfiniteQuery` requests `limit = page * 10` and keeps the new slice. `failedCount` is counted from loaded rows with status `failed`. `completed` maps to `paid`; `expired` stays `expired`. The list polls every 30s while any loaded row is `pending` (`refetchIntervalInBackground: false`).

`GET /bonuses/open` returns `total_payout`, `total_count`, and `batches` (`title`, `volume`, `list` of operation items including `email`, `purpose`, and `description`). The Bonuses to be paid table keeps each batch as a grouped row: Bonus Title / Member / Purpose / Description / Bonus / Address / Email / Action. One member shows those fields on the row; more than one expands and shows `-` for Purpose / Description / Address / Email until expanded. Description uses the same Description cell as Expense (http(s) URLs are links with a receipt icon before each URL; empty cells are `-`). Batch `volume` is the grouped-row USD total. Member `amount` / `symbol` and `list[].volume` (falls back to `amount`) drive the Bonus cell as `amount symbol ($volume)` on a one-member row and each expanded member. `list[].email` is mapped onto `BonusPendingMember` and carried into Pay Now. `status` `paying` / `processing` / `submitted` on any member shows Paying for that batch; anything else is Pay Now with that batch's `batch_id`. An empty batch list is the create-bonus empty state.

`GET /bonuses/history` is paginated (`page` / `pageSize`, max 100) and accepts `start_time` / `end_time` as Unix seconds. The Bonus History tab does not send those filters yet. `useBonusHistoryInfiniteQuery` loads the next page on scroll. Each execution item maps like Expense History (`name`, `purpose`, `description`, `destination_volume`, `recipient`, `status`, `destination_tx_hash`). History Status is Failed, Expired, Paid, or Pending. Failed and Expired rows with a numeric `execution_item_id` / `id` show **Pay Again** (`POST /payouts/retry`, `success_url` `{origin}/pay/bonus/history`).
...
Catalog rows map `id`, `category`, `name`, `icon`, `description`, `added`, `status`. Mappers read snake/camel through `asRecord` / `apiText` / `apiNumber`. Unknown `category` values are kept. Sidebar leaves are `added && status === "active"`. Add is `POST /organizations/{id}/operations` when `added` is false. After that, enable/disable is `POST .../operations` with `{ operation_id, enable }`. A successful add/enable/disable patches `queryKeys.operation.catalog` from the POST `OperationCatalogItem` (cancel any in-flight catalog GET first) so the Switch and sidebar update immediately; other operation queries are invalidated, not the catalog. Import still invalidates `queryKeys.operation.all`. Turning off the category currently on screen `replace`s to `/`.
...
Recent / open / history reuse the expense list shapes. Open `list[].email`, `purpose`, and `description` are mapped the same way as expenses. The Payments table columns match Open expense (Title / Name / Purpose / Description / Address / Email / Payout Preference / Amount). Recent infinite query requests `limit = page * 10` (max 100) and polls every 30s while any loaded row is `pending`. History Status is Failed, Expired, Paid, or Pending. Failed and Expired rows with a numeric `execution_item_id` / `id` show **Pay Again** (`POST /payouts/retry`, `success_url` `{origin}/pay/{category}/history`). History has no `search`. Export uses the same date filters without pagination. Filename comes from `Content-Disposition`, falling back to `operation-history.csv`.
...
| Method | Path | Auth | Body / Query | Data | API | Hook |
| --- | --- | --- | --- | --- | --- | --- |
| POST | `/v1/payroll/payouts/submit` | yes | `PayBatchSubmitParam` (`quote_id`, `quote_batch_id`, `tx_hash`; `tx_hash` may be `""` for a multisig that has no on-chain hash yet) | `PayrollPayoutSubmitResult` | `batchSubmit` | `PaymentByFormCard` (EOA one-shot) + `useMultisigWatchHost` (after a watched proposal succeeds) |
| POST | `/v1/payroll/payouts/retry` | yes | `PayrollPayoutRetryParam` | `PayrollPayment` | `retryPayrollPayout` | `useRetryPayrollPayoutMutation` / `useRetryPayoutItem` |
| GET | `/v1/payroll/executions/{execution_id}` | yes | `organization_id` | `PayrollExecution` | `getPayrollExecution` | `usePayoutExecutionPoll` |

Payment by form signs and broadcasts each payable quote batch, then calls `batchSubmit` once with `{ quote_id, quote_batch_id, tx_hash }`. Multisig success uses the on-chain hash when the watcher can read one, otherwise `""` (backend fills it after nearintents observes the deposit). Success returns `execution_id` and `notifyBatchPayoutCommitSuccess` starts the root `AppWatchLayout` poll. Failure toasts and is not retried; the on-chain batch still counts as paid so the form can advance. `enqueueBatchPayoutCommit` / `processAllPendingBatchPayoutCommits` remain in `batch-payout-commit-queue` but are unused. The host then polls `GET /executions/{execution_id}` every 5s without blocking the pay form. Same-form split batches keep one progress toast per execution (title `{formTitle}_Batch Payment n`). Starting a different form (`formKey`) dismisses those toasts and follows only the latest execution. Progress is an `info` toast (`{processed} / {total} Transactions are in progress...` plus View). Newly terminal list items toast `completed` / `failed` / `expired`. View on a toast goes to the matching history list (`/pay/payroll/history`, `/pay/expense/history`, `/pay/bonus/history`, or `/pay/{category}/history`) and dismisses only that toast. `finished: true` changes the copy to `{processed} / {total} Transactions completed`, keeps the toast 3s, then closes it, and invalidates that type's recent + history queries (Operations also invalidates `queryKeys.operation.all` on commit). Watch sessions and in-flight executions are restored from `sessionStorage` on refresh in the same tab; see [multisig.md](multisig.md).
