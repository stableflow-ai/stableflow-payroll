...
`GET /expenses/open` returns `total_payout`, `total_count`, and `batches` (`title`, `volume`, `count`, `list` of operation items including `email`). The Open expense table keeps each batch as a grouped row: one member shows the name, address, and payout preference; more than one expands. Batch `volume` is the header Amount; member `amount` / `symbol` / `network` are the payout. `list[].email` is mapped onto `ExpenseOpenRow` and carried into Pay Now. `status` `paying` / `processing` / `submitted` on any member shows Paying for that batch; anything else is Pay Now with that batch's `batch_id`.
...
`GET /bonuses/open` returns `total_payout`, `total_count`, and `batches` (`title`, `volume`, `list` of operation items including `email`). The Bonuses to be paid table keeps each batch as a grouped row: one member shows the name and address, more than one expands. `volume` is the USD total; member `amount` / `symbol` are the payout. `list[].email` is mapped onto `BonusPendingMember` and carried into Pay Now. `status` `paying` / `processing` / `submitted` on any member shows Paying for that batch; anything else is Pay Now with that batch's `batch_id`. An empty batch list is the create-bonus empty state.
...
Recent / open / history reuse the expense list shapes. Open `list[].email` is mapped the same way as expenses. Recent infinite query requests `limit = page * 10` (max 100) and polls every 30s while any loaded row is `pending`. History has no `search`. Export uses the same date filters without pagination. Filename comes from `Content-Disposition`, falling back to `operation-history.csv`.
