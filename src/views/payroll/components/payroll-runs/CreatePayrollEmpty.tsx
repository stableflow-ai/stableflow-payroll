...
export function CreatePayrollEmpty(props: {
  ...
  const { onAddPayroll, onImported, busy = false } = props;
  ...
  function applyValues(values: string[][]) {
    const parsed = parsePayrollImportRows(values, PAYROLL_FORM_MAX_ROWS);
    if (parsed.truncated) {
      toast.fail({ title: `Imported the first ${PAYROLL_FORM_MAX_ROWS} rows` });
    }
