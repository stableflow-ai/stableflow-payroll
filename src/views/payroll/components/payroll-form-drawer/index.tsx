...
export function PayrollFormDrawer(props: {
  ...
  onClose: () => void;
  onSave: (run: PayrollNextRun) => void;
}) {
  ...
  const [payDate, setPayDate] = useState<PayrollPayDate>(() => payDateFromLabel(initialPayDate));
  const [rows, setRows] = useState<PayrollFormRow[]>(() => [createEmptyPayrollFormRow()]);
  ...
  useEffect(() => {
    ...
    setRows(
      initialRows && initialRows.length > 0
        ? initialRows.map((row) => formRowFromRecipient(row, findByChainAndSymbol))
        : [createEmptyPayrollFormRow()],
    );
  }, [open]);
  ...
  useEffect(() => {
    if (!open) return;
    setRows((current) => refillPayrollFormTokens(current, findByChainAndSymbol));
  }, [open, tokens, findByChainAndSymbol]);

  const destRow = rows.find((row) => row.id === destRowId) ?? null;
  const canSave = isPayrollFormValid(rows);

  function patchRow(rowId: string, patch: Parameters<typeof patchPayrollFormRow>[1]) {
    setRows((current) =>
      current.map((row) =>
        row.id === rowId ? patchPayrollFormRow(row, patch, findByChainAndSymbol) : row,
      ),
    );
  }
      ...
      <div className="flex min-h-full flex-col">
          ...
          <div className="flex flex-col gap-6">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4">
              ...
              <Dropdown
                ...
                value={payDate}
                onChange={(value) => setPayDate(value as PayrollPayDate)}
                options={[...PAYROLL_PAY_DATE_OPTIONS]}
              />
            </div>
        ...
        <div className="sticky bottom-0 -mx-6 flex justify-end gap-4 border-t border-black/10 bg-[#FDFDFD] px-6 py-5 sm:-mx-8 sm:px-8">
          ...
          <Button
            ...
            disabled={!canSave}
            onClick={() => {
              // TODO(api): persist the payroll draft when the contract exists.
              onSave(formRowsToNextRun(rows, payDate));
            }}
          >
            Save
          </Button>
