...
import { Button, type ButtonProps } from "@stableflow/pay-ui/button";
...
export function ExpenseImportCsvButton(props: {
  ...
  function applyValues(values: string[][]) {
    const parsed = parseExpenseImportRows(values, EXPENSE_FORM_MAX_ROWS);
    if (parsed.truncated) {
      toast.fail({ title: `Imported the first ${EXPENSE_FORM_MAX_ROWS} rows` });
    }
