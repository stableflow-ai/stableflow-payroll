...
export type { PartnerApiKey, PartnerRegistration };
...
export function usePartner() {
  if (!MOCK_ENABLED.partner) {
    throw new Error("Partner mock is disabled. Wire TanStack Query before turning MOCK_ENABLED.partner off.");
  }

  const [isPartner, setIsPartner] = useState(() => getIsPartner());
  const [apiKeys, setApiKeys] = useState<PartnerApiKey[]>(() => getPartnerApiKeys());

  useEffect(() => {
    return subscribePartner(() => {
      setIsPartner(getIsPartner());
      setApiKeys([...getPartnerApiKeys()]);
    });
  }, []);
