...
- **Released surface is Auth (`/login`, `/register`, `/invite/:orgId`, `/auth/google/bind`, `/register/google`, `/invite/:orgId/google`), `/` (Overview), `/pay/*`, `/team`, `/history`, and `/setting` (including `/setting/slack/callback`).** `/howitworks` and `/docs` are live public pages.
